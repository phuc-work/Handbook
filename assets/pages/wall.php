...
foreach($posts as $post){
    ...
    <ul class="dropdown-menu" aria-labelledby="option<?=$post['id']?>">
        <?php if ($post['uid'] == $user['id']) { ?>
            <li><a class="dropdown-item" href="assets/php/actions.php?deletepost=<?=$post['id']?>"><i class="bi bi-trash-fill"></i>Xóa bài viết</a></li>
            <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#editPostModal<?=$post['id']?>"><i class="bi bi-pencil-fill"></i>Chỉnh sửa bài viết</a></li>
        <?php } ?>
        <li><a class="dropdown-item" href="#" onclick="reportPost(<?=$post['id']?>)"><i class="bi bi-flag-fill"></i> Báo cáo vi phạm</a></li>
    </ul>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="assets/php/actions.php" method="POST">
                    ...
                    <textarea name="post_content" class="form-control" rows="3"><?= htmlspecialchars($post['post_text']) ?></textarea>
            </div>
...
<div id="comments-section-<?=$post['id']?>">
    ...
    <?php if ($totalComments > 3) { ?>
        <div class="text-center mt-2">
            <a href="javascript:void(0);" class="toggleCommentsLink" style="color: blue; text-decoration: underline;" data-post-id="<?=$post['id']?>">Xem thêm</a>
        </div>
    <?php } ?>
</div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const toggleLinks = document.querySelectorAll('.toggleCommentsLink'); // Chọn tất cả link "Xem thêm"

        toggleLinks.forEach(link => {
            const postId = link.dataset.postId; // Lấy ID bài đăng từ data-post-id
            const hiddenComments = document.querySelectorAll(`#comments-section-${postId} .comment-item.d-none`); // Chọn bình luận ẩn của bài đó
...
foreach($follow_suggestions as $suser){
    ?>
            <div class="d-flex justify-content-between">
                ...
                <div class="d-flex align-items-center">
                    <button class="btn btn-sm btn-primary followbtn" data-user-id='<?=$suser['id']?>'>Theo dõi</button>
