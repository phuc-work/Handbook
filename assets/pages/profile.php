...
if($user['id']!=$profile['id'] && !checkBS($profile['id'])){
  ...
  <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat(<?=$profile['id']?>)"><i class="bi bi-chat-fill"></i> Nhắn tin</a></li>
                                <li><a class="dropdown-item " href="assets/php/actions.php?block=<?=$profile['id']?>&username=<?=$profile['username']?>"><i class="bi bi-x-circle-fill"></i> Chặn</a></li>
                            </ul>
                        </div>
    <?php
}
...
<a class="btn btn-sm btn-primary"><i class="bi bi-file-post-fill"></i> <?=count($profile_post)?> bài đăng</a>
<a class="btn btn-sm btn-primary <?=count($profile['followers'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#follower_list"><i class="bi bi-people-fill"></i> <?=count($profile['followers'])?> người theo dõi</a>
<a class="btn btn-sm btn-primary <?=count($profile['following'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#following_list"><i class="bi bi-person-fill"></i> <?=count($profile['following'])?> người đang theo dõi</a>
...
<?php
   ...
   ?>
<button class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?=$profile['id']?>' >Bỏ theo dõi</button>
...
foreach($profile_post as $post){
  ...
  ?> 
                ...
                <div style="font-size:small" class="text-muted">Đã đăng <?=show_time($post['created_at'])?> </div> 
...
<div class="d-flex align-items-center p-2">
  ...
  <div class="input-group p-2 border-top">
                            <input type="text" class="form-control rounded-0 border-0 comment-input" placeholder="nói điều gì đó.."
